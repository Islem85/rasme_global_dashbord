...
# Maximum dots to render on the Leaflet map. Set above the current mappable
# volume (~24.5 k) so EVERY country is shown — small ones (Egypt, RCA) were
# being dropped when a 5 000 cap truncated the result set. The map clusters
# markers (see mapper.leaflet_map), so this volume stays responsive.
...
# Pull all data once — needed by both the hero (site count) and Pillar 1.
kpi      = kpi_overview(filters)
dlv      = delivery_kpis(filters)

# ── HERO ────────────────────────────────────────────────────────────────────
# Site count is sourced from the same kpi.get("sites") used by Pillar 1 so the
# hero figure stays in sync with « Périmètre du portefeuille suivi ».
hero(
...
# Coverage data computed once and reused by Pillar 1 + Pillar 3 so the math
# closes: Projets cartographiés (Pillar 3) = Projets actifs (Pillar 1) =
# n_internal, ventilé en « avec correspondance BAD » + « sans correspondance ».
try:
    _bad_o = bad_overview()
    # Scope by the region filter (list of country slugs) so "Projets actifs"
    # and the Pillar 3 coverage row reflect the selected region, not the total.
...
kpi_row([
    ...
    kpi_card(
        ...
        # Aligné sur Pilier 3 : codes SAP distincts (RASME) pour que la math
        # ferme Projets actifs = cartographiés + non cartographiés.
...
#   ventilés en « avec / sans correspondance BAD ».
...
#
# _bad_o et _cov sont déjà calculés en haut du fichier — on les réutilise
# pour garantir la cohérence avec Pilier 1.
if _cov_ok:
    _snap    = _bad_o.get("last_refresh")
    _snap_s  = _snap.strftime("%Y-%m-%d") if hasattr(_snap, "strftime") else (str(_snap) if _snap else "—")
    _rate    = _cov["mapping_rate"]
    ...
    # Row 1 — Vue RASME : projets cartographiés (= projets actifs, Pilier 1),
    ...
    _n_internal      = _cov.get("n_internal") or 0
    _n_internal_only = _cov.get("n_internal_only") or 0
    ...
    kpi_row([
        kpi_card(
            ...
            explainer=("Total des projets actifs suivis par RASME (= Projets actifs, Pilier 1)."
                       if lang == "FR" else
                       "Total active projects monitored by RASME (= Active projects, Pillar 1)."),
        ),
        kpi_card(
    ...
    # Row 2 — Couverture du portefeuille BAD ACTIF (Approved + Ongoing).
    kpi_row([
        kpi_card(
            ("Projets actifs (BAD)" if lang == "FR" else "Active projects (BAD)"),
            fmt_int(_cov["n_portfolio"]),
            icon="globe",
            explainer=("Projets BAD aux statuts Approved + Ongoing."
                       if lang == "FR" else
                       "BAD projects with status Approved + Ongoing."),
        ),
        kpi_card(
            ("Actifs cartographiés" if lang == "FR" else "Mapped active"),
            fmt_int(_cov["n_mapped"]),
            tone="success", icon="completion",
            explainer=("Projets actifs avec au moins un projet interne correspondant."
                       if lang == "FR" else
                       "Active projects with at least one matching internal project."),
        ),
        kpi_card(
            ...
            icon="risk",
            explainer=("Projets actifs sans monitoring interne — couverture à compléter."
                       if lang == "FR" else
    ...
    # Row 3 — Visibilité Completion + Cancelled (exclus du taux de couverture).
    kpi_row([
        kpi_card(
            ("Clôturés (BAD)" if lang == "FR" else "Completion (BAD)"),
            fmt_int(_bad_o.get("completion") or 0),
            icon="check",
            explainer=("Projets BAD au statut Completion."
                       if lang == "FR" else
                       "BAD projects with status Completion."),
        ),
        kpi_card(
...
if pts.empty:
    ...
    leaflet_map(
        pts, lang,
        height=620,
        center=(4.0, 20.0),    # centred on the African continent
        zoom=3,
        point_cap=MAP_POINT_LIMIT,
        key="global_leaflet",
    )
    # Sites without valid GPS coordinates can't be plotted — surface the gap so
    # the map count is never mistaken for the "Sites total" KPI (the map is a
    # strict subset: total = géolocalisés + sans GPS).
    _total_sites = int(kpi.get("sites") or 0)
    _no_gps = max(_total_sites - len(pts), 0)
    _gps_note_fr = (f" · {_no_gps:,} site(s) sans coordonnées GPS (non affichés)"
                    if _no_gps else "")
    _gps_note_en = (f" · {_no_gps:,} site(s) without GPS coordinates (not shown)"
                    if _no_gps else "")
    st.caption(
...
with col_a:
    ...
    if not cs.empty:
        ...
        fig = px.bar(
            ...
            labels={"country_label": "", "sites": t("sites_total", lang),
...
with col_b:
    ...
    if not sec.empty:
        # Translate sector labels to the active language.
...
if trend.empty:
    ...
    # Defensive filter — any NaT month or NaN counter would render as
    # "undefined" in the Plotly axis or hover.
...
# Affiché en bas pour ne jamais bloquer le rendu des piliers supérieurs si
# le calcul (fuzzy matching ~10s à froid) est lent ou échoue.
try:
    with st.spinner(("Compilation du tableau pays-par-pays…" if lang == "FR"
                     else "Compiling country-by-country table…")):
        _by_country = coverage_by_country()
    # Follow the region filter: keep only the selected region's countries.
    ...
    if not _by_country.empty:
        pillar_header(
            eyebrow=("Couverture Projets BAD · détail" if lang == "FR"
                     else "Projets BAD coverage · detail"),
            title=("Couverture par pays" if lang == "FR" else "Coverage by country"),
            description=("Détail pays-par-pays : nombre de soumissions kobo, "
                         "portefeuille BAD, projets cartographiés et taux."
                         if lang == "FR" else
                         "Per-country detail: kobo submissions, BAD portfolio, "
                         "mapped projects and rate."),
        )
        _disp = _by_country.copy()
        _disp["country"] = _disp["country_slug"].map(lambda s: country_label(s, lang))
        # Compute total mapped (all statuses) per country, then keep the
        # numeric rate as a float so we can colour it via Styler.
        ...
        # Colour the % column: green ≥80, amber 50-79, red <50, grey if 0
        # portfolio. Streamlit renders pandas Styler natively.
...
pillar_header(
    ...
    description=("Les 4 lectures à retenir de la période sélectionnée."
                 if lang == "FR" else
                 "Four take-aways from the selected period."),
)
...
if not status_d.empty:
    risky = status_d[status_d["status"].isin(
        ["Stalled", "Stalled/ suspended", "Abandoned", "Suspended", "Canceled"]
    )]["sites"].sum()
    total = int(status_d["sites"].sum() or 0)
    if risky:
        insight(
            t("insight_at_risk", lang,
              n=fmt_int(risky),
              pct=f"{(100 * risky / max(total, 1)):.1f}"),
            tone="danger",
        )
